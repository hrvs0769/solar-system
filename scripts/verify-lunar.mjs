...
const expect=['COUNTDOWN','IGNITION','LIFTOFF','STAGE_SEP','SPHERE','EARTH_ORBIT','DEPLOY','TRANSFER','LOI','LUNAR_ORBIT','LANDING','LANDED'];
...
const surface=await p.evaluate(()=>({ min:window.__SS?.cameraRig?.controls?.minDistance, max:window.__SS?.cameraRig?.controls?.maxDistance,
  reset:!!document.getElementById('mission-reset') }));
ok('月面观察可大范围缩放且有复位视角按钮', surface.max>=3 && surface.reset===true, JSON.stringify(surface));
